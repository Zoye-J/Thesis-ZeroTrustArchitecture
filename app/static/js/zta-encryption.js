...
class ZTAEncryption {
    ...
    async decryptFromAgent(encryptedData) {
        ...
        try {
            ...
            if (typeof encryptedData === 'string' && encryptedData.trim().startsWith('{')) {
                ...
                // Backend hybrid format (with type and tag fields)
                if (parsed.type === 'hybrid' && parsed.tag) {
                    console.log('🔐 Detected backend hybrid encryption with separate tag');
                    return await this.hybridDecryptBackend(parsed);
                }
                
                // Frontend hybrid format
    ...
    async hybridDecryptBackend(encryptedPackage) {
        console.log('🔐 Using backend hybrid decryption with separate tag');
        ...
        try {
            ...
            const tag = this.base64ToArrayBuffer(encryptedPackage.tag);  // ← GET SEPARATE TAG
            
            console.log(`  encrypted_data: ${encryptedData.byteLength} bytes`);
            console.log(`  tag: ${tag.byteLength} bytes`);
            
            // For Web Crypto, we need to append the tag to the encrypted data
            ...
            console.log(`  combined_data: ${combinedData.length} bytes`);
